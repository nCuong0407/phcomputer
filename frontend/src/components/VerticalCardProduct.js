...
import fetchCategoryWiseProduct from "../helpers/fetchCategoryWiseProductRandom";
...
const VerticalCardProduct = ({ category, heading }) => {
  ...
  const [visibleCount, setVisibleCount] = useState(10); // Hiển thị 10 sản phẩm đầu
  ...
  const handleShowMore = () => {
    setVisibleCount((prev) => prev + 10); // Hiển thị thêm 10 sản phẩm
  };
          ...
          (product, index) => {
            return loading ? (
              ...
              <Link
                ...
                key={index}
                className="bg-white p-3 rounded-md shadow hover:shadow-md transition duration-300 flex flex-col"
              >
                <div className="h-18 flex justify-center items-center mb-2">
                  <img
                ...
                <h3 className="font-medium text-sm mb-1 break-words leading-5">
                  {product.productName}
                </h3>
